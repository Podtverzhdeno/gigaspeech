...
_CRITIQUE_TEMPLATE = """Ты - выпускающий редактор. Оцени речь.
...
Правила оценки:
- Соответствует ли текст теме и тайму?
- Использованы ли цитаты из материалов с явным указанием источника?
- Нет ли выдуманных цитат, которых нет в материалах?
- Логичная ли структура?

Если речь готова - верни РОВНО одну строку: APPROVED

Если есть замечания - верни конкретные пункты, что переписать. Только конструктивная
критика, без общих фраз и похвалы."""
