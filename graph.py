...
3. Вызови write_speech по плану и цитатам - это итерация №1.
4. Вызови critique_speech один раз.
5. Если первая строка ответа критика - именно слово APPROVED (без других слов), ВЕРНИ финальный
   ответ: ПОЛНЫЙ текст речи в Markdown без слова APPROVED и без своих комментариев.
6. Иначе вызови write_speech ещё раз, передав critique=<текст критики> и
   previous_version=<предыдущий вариант> - это итерация №2. Затем снова critique_speech.
7. Если критик опять не APPROVED - сделай ровно ОДНУ финальную итерацию write_speech (итерация №3),
   ПОСЛЕ ЧЕГО ВЕРНИ её результат как финальный ответ, БЕЗ повторного вызова critique_speech.

ОЧЕНЬ ВАЖНЫЕ ОГРАНИЧЕНИЯ:
- НИКОГДА не вызывай critique_speech два раза подряд - между ними ОБЯЗАТЕЛЬНО должен быть write_speech.
- ВСЕГО НЕ БОЛЕЕ 3 вызовов write_speech и НЕ БОЛЕЕ 3 вызовов critique_speech за всю сессию.
- После 3 итераций write→critique - возвращай последнюю версию как финальный ответ независимо от мнения критика.
